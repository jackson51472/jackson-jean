...
export class GameState {
  constructor(canvas, hudElem, upgradeElem) {
    ...
    this.upgradePanel = new UpgradePanel(upgradeElem, this.upgrades, () =>
      this.applyUpgrade()
    )
    this.gameOver = false
    this.lastTime = 0 // delta time
  ...
  async loadAssets() {
    ...
    // Prioriza miner.png; fallback para mine.png
    this.assets.mine = await loadImg("assets/images/miner.png")
    if (!this.assets.mine) {
      this.assets.mine = await loadImg("assets/images/mine.png")
    }
    // Tenta carregar coin.gif; se falhar, usa coin.png
    this.assets.coin = await loadImg("assets/images/coin.gif")
    if (!this.assets.coin) {
      this.assets.coin = await loadImg("assets/images/coin.png")
    }
    // Prioriza novo nome 'spitesheet.png'; fallback para 'player_spritesheet.png' e 'spritesheet.png'
    this.assets.playerSheet = await loadImg("assets/images/spitesheet.png")
    if (!this.assets.playerSheet) {
      this.assets.playerSheet = await loadImg(
        "assets/images/player_spritesheet.png"
      )
    }
    if (!this.assets.playerSheet) {
      this.assets.playerSheet = await loadImg("assets/images/spritesheet.png")
    }
  }
  ...
  addListeners() {
    window.addEventListener("keydown", (e) => {
      ...
      if (e.code === "Space") {
        e.preventDefault()
        this.submarine && this.submarine.shoot()
      }
  ...
  applyUpgrade() {
    ...
    if (
      this.upgradePanel.selected === "engine" &&
    ...
    if (
      this.upgradePanel.selected === "shield" &&
  ...
  update(dt) {
    ...
    if (this.spawnTimer >= this.spawnInterval) {
      ...
      this.mines.push(
        new Mine(this.canvas.width, y, this.mineSpeed, this.assets)
      )
      ...
      if (Math.random() < 0.3) {
        this.coins.push(
          new Coin(
            this.canvas.width,
            Math.random() * (this.canvas.height - 24),
            this.mineSpeed,
            this.assets
          )
        )
